...
### Радио
- `/play name:[название]` — воспроизведение радиостанции
...
Существует три способа указания токена:

1. Аргумент командной строки
2. Переменная окружения `DISCORD_TOKEN`
3. Создать текстовый файл TOKEN.txt, внутри вставить токен.

> ⚠️ **Внимание! Для корректной работы автообновления обязательно запускайте бота через скрипты** `run.bat` **(Windows)** или **`run.sh` (Linux/Mac)**. Если запускать напрямую через JAR, обновление скачивается, но не применяется!

### Через JAR-файл

```bash
java --enable-native-access=ALL-UNNAMED -Dfile.encoding=UTF-8 -Dconsole.encoding=UTF-8 -jar DiscordjBot-jar-with-dependencies.jar [token]
```

> ❗️ При запуске напрямую через JAR автообновление не будет работать корректно. Рекомендуется использовать скрипты запуска ниже.
...
4. **Воспроизведение радиостанции**:
   - Используйте команду `/play [название радио из списка]` - бот автоматически подключится к голосовому каналу
...
### Основные библиотеки
- JDA (Java Discord API) 6.3.2 - основной фреймворк для работы с Discord
...
- Для запуска бота на Windows обязательно используйте скрипт `run.bat` (только так работает автообновление и корректная кодировка)
- Для запуска бота на Linux/Mac обязательно используйте скрипт `run.sh` (только так работает автообновление и корректное отображение русских символов в логах)
- Если запускать напрямую через JAR, автообновление не будет работать!
